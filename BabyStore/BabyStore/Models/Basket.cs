...
namespace BabyStore.Models
{
    public class Basket
    {
        ...
        public void UpdateBasket(List<BasketLine> lines)
        {
            foreach (var line in lines)
            {
                var basketLine = db.BasketLines
                    .FirstOrDefault(b => b.BasketId == BasketId && b.ProductId == line.ProductId);
                if (basketLine != null)
                {
                    if (line.Quantity == 0)
                    {
        ...
        public void MigrateBasket(string userName)
        {
            ...
            //find if the user already have a basket or not and store it in memory using ToList()
        ...
        public decimal CreateOrderLines(int orderId)
        {
            ...
            foreach (var basketLine in basketLines)
            {
                ...
                orderTotal += (basketLine.Quantity * basketLine.Product.Price);
